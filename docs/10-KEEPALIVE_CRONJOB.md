...
An internal `node-cron` running *inside* the Render service cannot keep the service awake. When Render spins the container down, the cron stops running along with the rest of the process. By the time the cron wakes back up (when someone hits the server), the cold-start has already happened. Documenting this because it is a common mistake.
...
An internal cron is still useful — for scheduled work that happens *while the service is up*. We use `node-cron` in the API for:
