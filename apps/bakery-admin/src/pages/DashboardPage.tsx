    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          icon={<IconProductTrending size="lg" color="default" alt="" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Revenue by Day */}
        <div>
          <LineChart data={revenueChartData} title="Revenue Trend" color="#10b981" />
        </div>

        {/* Orders by Status */}
        <div>
          <PieChart data={statusChartData} title="Orders by Status" />
        </div>

        {/* Top Products */}
        <div className="lg:col-span-2">
          <BarChart data={productChartData} title="Top 5 Products by Units Sold" color="#3b82f6" />
        </div>
      </div>
      ...
      {metrics.topProducts.length > 0 && (
        <div className="rounded-lg border border-platform-border bg-platform-surface overflow-hidden">
          <div className="p-6 border-b border-platform-border">
            <h3 className="text-lg font-semibold text-platform-fg">Top Products</h3>
          </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-platform-border">
              {metrics.topProducts.map((product) => (
                <tr key={product.productId} className="hover:bg-platform-bg/50 transition-colors">
                  <td className="px-6 py-3 text-sm text-platform-fg font-medium">
                    {product.productName}
                  </td>
                  <td className="px-6 py-3 text-sm text-platform-fg">{product.unitsSold}</td>
